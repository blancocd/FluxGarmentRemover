...
def prepare_data(dataset_dir, scan_name):
    transforms_path = os.path.join(dataset_dir, scan_name, "transforms_train.json")

    with open(transforms_path, "r") as f:
        transforms = json.load(f)

    odd_dir = os.path.join(dataset_dir, f"odd_views", scan_name)
    even_dir = os.path.join(dataset_dir, f"even_views", scan_name)
    ...
    for frame in transforms["frames"]:
        ...
        original_img_path = os.path.join(dataset_dir, scan_name, "images",
                                            f"train_{img_num:04d}.png")
        if not os.path.exists(original_img_path):
            continue
        
        frame["file_path"] = f"./images/train_{img_num:04d}"
    ...
    even_transforms = {**transforms, "frames": my_frames[::2]}
    odd_transforms = {**transforms, "frames": my_frames[1::2]}
...
def run_mvfit(mvg_bench_dir,  dataset_dir, scan_name):
...
def run_evaluation(mvg_bench_dir):
...
def main():
    ...
    if not args.skip_data_prep:
        prepare_data(args.data_dir, scan_name)
    if not args.skip_mvfit:
        run_mvfit(args.mvg_bench_dir, args.data_dir, scan_name)
    if not args.skip_eval:
        if args.scan_index != 0:
            return
        run_evaluation(args.mvg_bench_dir)
