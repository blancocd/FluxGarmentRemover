...
def main():
    ...
    for gen_path in train_paths:
        train_image = Image.open(gen_path).convert("RGB")
        ...
        # Save segmentation mask and RGBA image
        ...
        gen_np = np.array(train_image)
        alpha = np.where(pred != 0, 255, 0).astype(np.uint8)
